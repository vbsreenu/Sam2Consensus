...
int main(int argc, char **argv){ 
	...
	for(i=1; i<argc; i++){
		...
		if(strcmp(argv[i],"-f")==0){ f=1;}					// Extended output
	...
	if(e==1){ 	// Detailed output
		...
		for(i=0; i<GenomeLen; i++){ 
			GetConsensusDetailed(A[i],T[i],G[i],C[i],N[i],Aq[i],Tq[i],Gq[i],Cq[i], d, &consensus,&coverage,&af,&tf,&gf,&cf,&nf,&aq,&tq,&gq,&cq,f);
...
void Usage(){
	...
	printf("\t-f integer (Print nucleotide frequencies at each position. default: NO)\n");
